		...
		<div class="row">
			<div class="col-sm-6">
				...
				<!-- <form action="addDm" method="POST"> -->
					  <div class="form-group text-xs-center" >
					    ...
					    <input name="name_dmma" type="text" class="form-control" id="name_dmma" aria-describedby="emailHelp" placeholder="nhâp tên danh mục">
					  </div>	 
			...
			<div class="col-sm-6">
				<div class="jumbotron jumbotron-fluid text-xs-center">
					<div class="container">
						<h1 class="display-5">Danh Sách Mục Món Ăn</h1>
						<p class="lead">Các Danh Mục Món Ăn Đã Thêm</p>
					</div>
				</div>
				<div class="ajax-add">
				  ...
				  <div class="card-block">
				    <div class="col-sm-12">
				    	<blockquote class="card-blockquote">    	
							<div class="name_dmma">
								<h5 class="card-text"><?php echo $value['name_dmma'] ?></h5>
							</div>
							 <fieldset class="form-group hidden-xs-up jquery_dm" col-sm-12>
					    		<input type="text" class="form-control" value="<?php echo $value['name_dmma'] ?>" >
					    	</fieldset >
	...
	<script>
		...
		$(function(){
			...
			$('body').on('click', '.nutedit', function(event) {
				...
				$(this).parent().parent().find('.name_dmma').addClass('hidden-xs-up');
			...
			$('body').on('click', '.nutluu', function(event) {
				...
				hienthi2 = $(this).parent().parent().parent().find('.name_dmma').html(noidung).removeClass('hidden-xs-up');

				$.ajax({
					url: duongdan + 'Admin/do_editDmma',
					type: 'POST',
					dataType: 'json',
					data: {
							id     : giatriid,
							name_dmma: giatriname},
				})
				.done(function() {
					console.log("success");
				})
		...
		$('.nutxulyajax').click(function(event) {
			...
			.always(function(res) {
				...
				noidung += '<div class="name_dmma">';
				noidung += '<p class="card-text"> '+ $('#name_dmma').val()+'</p>';
				noidung += '</div>';
				noidung += '<fieldset class="form-group hidden-xs-up jquery_dm">';
				noidung += '<input type="text" class="form-control " value="<?php echo $value['name_dmma'] ?>" >';
		...
		$('body').on('click', '.nutxoa', function(event) {
			...
			$.ajax({
				url: duongdan + 'Admin/deleteDmma/'+id,
				type: 'POST',
				dataType: 'json'
